...
func TestAddContributionType(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			contributionType: "",
			wantErr:          true,
			errMsg:           ErrEmptyContributionType.Error(),
		},
		{
			name:             "invalid contribution type",
			contributionType: "invalid",
			wantErr:          true,
			errMsg:           ErrInvalidContributionType.Error(),
		},
		{
			name:             "duplicate contribution type",
			contributionType: "development",
			wantErr:          true,
			errMsg:           ErrDuplicateContributionType.Error(),
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			found := false
			for _, t := range asset.ContributionTypes {
				if t == tt.contributionType {
					found = true
					break
				}
			}
			if !found {
				t.Errorf("Contribution type %q not found in list", tt.contributionType)
			}
			if asset.Version != 2 {
				t.Errorf("Expected version 2, got %d", asset.Version)
			}
